...
/**
 * 絵文字選択用のサジェストコンポーネント
 * AbstractInputSuggestパターンを使用して入力補完を提供
 */
export class EmojiSuggest extends AbstractInputSuggest<EmojiItem> {
	...
	renderSuggestion(item: EmojiItem, el: HTMLElement): void {
		el.addClass("extract-selection-emoji-suggestion");

		const container = el.createDiv({ cls: "extract-selection-emoji-container" });

		// 絵文字アイコン（大きめ表示）
		container.createSpan({
			text: item.emoji,
			cls: "extract-selection-emoji-icon",
		});

		// テキスト部分
		const textContainer = container.createDiv({ cls: "extract-selection-emoji-text" });

		// 名前
		textContainer.createDiv({
			text: item.name,
			cls: "extract-selection-emoji-name",
		});

		// カテゴリ（小さめ表示）
		textContainer.createDiv({
			text: item.category,
			cls: "extract-selection-emoji-category",
		});
	}
