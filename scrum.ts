...
const scrum: ScrumDashboard = {
  ...
  retrospectives: [
    ...
    { sprint: 21, improvements: [{ action: "基盤先行・統合後行の段階的実装アプローチ継続", timing: "sprint", status: "completed", outcome: "Sprint 18-20で作成した基盤（NoteTypeModal/AliasInputModal/NoteCreatorService）を効果的に再利用し、Create New Noteコマンドを迅速に実装完了" }, { action: "i18n翻訳キー事前追加パターン継続", timing: "sprint", status: "completed", outcome: "Sprint 18で確立したパターンを継続。コマンド実装時にi18n翻訳キーが既に存在しているため、キー参照が容易で実装がスムーズ" }, { action: "再利用可能なメソッド設計（createNoteAndOpen）", timing: "sprint", status: "completed", outcome: "createNoteAndOpen()メソッドをNoteCreatorServiceに切り出すことで、今後のExtract to Note（PBI-022）でも再利用可能な設計を実現" }, { action: "設定連動による一貫したUX維持", timing: "sprint", status: "completed", outcome: "showEmojiInCommands設定フラグで絵文字表示を制御し、全コマンドで一貫したUX体験を提供" }] },
  ],
};
