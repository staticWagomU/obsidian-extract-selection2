...
const scrum: ScrumDashboard = {
  ...
  completed: [
    ...
    { number: 9, pbi_id: "PBI-011", goal: "Permanent Note接続率統計表示", status: "done", subtasks: [{ test: "OrphanDetectorService.getStats()（型定義: interface OrphanStats { total: number; orphans: number; connected: number; connectionRate: number; }、全permanentノート取得→getOrphanPermanentNotes()で孤立数→connected = total - orphans→connectionRate計算）", implementation: "src/services/orphan-detector-service.ts,src/types/index.ts", type: "behavioral", status: "completed", commits: [{ hash: "151f9ac", message: "feat(PBI-011): implement OrphanDetectorService.getStats()", phase: "green" }], notes: [] }, { test: "OrphanView.onOpen()でOrphanDetectorService.getStats()呼び出し、ヘッダーに統計情報表示（\"📊 接続率: X% (Y / Z 件が未接続)\"形式、リフレッシュ時に統計更新）", implementation: "src/ui/views/orphan-view.ts", type: "behavioral", status: "completed", commits: [{ hash: "374cc68", message: "feat(PBI-011): integrate stats display in OrphanView", phase: "green" }], notes: [] }] },
  ],
